...
/************************************************************************************/
union BreathingParametersT BreathingParameters;
uint8_t command_sended;
...
void function_send(uint8_t slave_address, uint8_t comando)
{
  ...
  for (i = 0; i < 2; i++)
  {
    ...
    Wire.write(BreathingParameters.BUFFER[pos]); // send data frame
  }
...
{
  int i;
  Wire.requestFrom(slave_address,(uint8_t)13); // request SIZE_DATA bytes from slave device SLAVE_ADDRESS
  for (i = 10; i < 23; i++)
  {
